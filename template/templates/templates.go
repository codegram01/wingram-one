...
func ParsePageTemplates() (map[string]*template.Template, error) {
	const tmplSource = "static/tmpl"
	fsys := template.TrustedFSFromTrustedSource(template.TrustedSourceFromConstant(tmplSource))
	...
	*/
	...
	for _, set := range htmlSets {
		...
		// add main page
		// frontend/[component]/*.tmpl
		// frontend/[component]/[component]/*.tmpl
		for _, f := range set {
